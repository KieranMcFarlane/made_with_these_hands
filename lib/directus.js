...
function directusAssetUrl(file) {
  ...
  if (typeof file === 'string') {
    ...
    return `${process.env.DIRECTUS_URL.replace(/\/$/, '')}/assets/${file}`;
  }
  if (typeof file === 'object' && file.id) {
    return `${process.env.DIRECTUS_URL.replace(/\/$/, '')}/assets/${file.id}`;
  }
  return null;
}
...
function normalizePage(item) {
  const blocks = Array.isArray(item.blocks) ? item.blocks.map(normalizeBlock).filter(Boolean) : [];
...
export async function getMwthDirectusData() {
  ...
  const [makers, products, episodes, posts, sections, pages] = await Promise.all([
    client.request(readItems(collections.makers, { fields: ['*'], filter, limit: -1 })),
    client.request(readItems(collections.products, { fields: ['*'], filter, limit: -1 })),
    client.request(readItems(collections.episodes, { fields: ['*'], filter, limit: -1 })),
    readOptionalItems(client, collections.posts, { fields: ['*'], filter, limit: -1 }),
    readOptionalItems(client, collections.sections, { fields: ['*'], filter, limit: -1 }),
    readOptionalItems(client, collections.pages, { fields: ['*', 'blocks.*', 'blocks.item.*'], filter, limit: -1 }),
  ]);
