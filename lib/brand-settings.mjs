...
export const DEFAULT_BRAND_RECORDS = [
  ...
  },
  {
    ...
    value: {
      ...
      presentation: {
        fields: {
          surface: { default: 'paper', choices: ['paper', 'muted', 'ink'] },
          tone: { default: 'editorial', choices: ['editorial', 'feature', 'restrained'] },
          contrast: { default: 'standard', choices: ['standard', 'high'] },
          image_focus: { default: 'center', choices: ['center', 'top'] },
          spacing: { default: 'standard', choices: ['compact', 'standard', 'generous'] },
        },
        natural_language: {
          darker: { surface: 'ink' },
          softer: { surface: 'muted', tone: 'restrained' },
          featured: { tone: 'feature' },
          quieter: { tone: 'restrained' },
          'higher contrast': { contrast: 'high' },
          'more breathing room': { spacing: 'generous' },
          tighter: { spacing: 'compact' },
          'keep faces visible': { image_focus: 'top' },
          portrait: { image_focus: 'top' },
        },
      },
      spacing: {
        base_unit_px: 4,
...
export function brandFromRecords(records = []) {
  ...
  for (const record of records) {
    if (record?.setting_key && record.value && typeof record.value === 'object') {
      brand[record.setting_key] = record.setting_key === 'component_contract'
        ? { ...brand.component_contract, ...record.value, presentation: record.value.presentation || brand.component_contract.presentation }
        : record.value;
    }
  }
